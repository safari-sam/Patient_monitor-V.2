...
impl Database {
    ...
    pub async fn get_reading_by_id(
        &self,
        id: i64,
    ) -> Result<Option<SensorEvent>, Box<dyn std::error::Error>> {
        let client = self.pool.get().await?;
    ...
    pub async fn get_alert_summary(&self) -> Result<AlertSummary, Box<dyn std::error::Error>> {
        ...
        let total: i64 = client
            .query_one("SELECT COUNT(*) FROM sensor_data", &[])
            .await?
            .get(0);

        let falls: i64 = client
            .query_one(
    ...
    async fn calculate_longest_still_period(
        ...
        for row in &rows {
            ...
            } else if let Some(start_time) = current_still_start {
